...
export function authenticateRequest(req, downstream) {
  ...
  if (xApiKey && safeEqual(xApiKey, downstream.api_key)) {
    return { ok: true };
  }
  ...
  if (authHeader) {
    ...
    if (scheme && scheme.toLowerCase() === 'bearer' && token && safeEqual(token, downstream.api_key)) {
      return { ok: true };
    }

    // 兜底：直接拿整个请求头去匹配
    const apiKey = authHeader.replace(/^Bearer\s+/i, '').trim();
    if (apiKey && safeEqual(apiKey, downstream.api_key)) {
      return { ok: true };
    }
  }
