  ...
  for (const item of input) {
    if (item.role) {
      const c = Array.isArray(item.content)
        ? (() => {
            const textParts = item.content.filter(ci => ci.type === 'input_text').map(ci => ci.text).filter(Boolean);
            const images = item.content.filter(ci => ci.type === 'image_file');
            // 纯文本 → 合并为字符串
            ...
            for (const img of images) {
              result.push({ type: 'image_url', image_url: { url: img.image_file?.url || '' } });
            }
            return result;
          })()
