...
/**
 * 从请求体中剥离所有 thinking/reasoning 相关参数。
 * 当下游配置了 force_disable_thinking 时调用，
 * 在翻译前（原文）和翻译后（目标格式）各执行一次，确保上游不会收到任何思考参数。
 */
export function stripThinkingParams(body, provider) {
  if (!body) return body;
  if (provider === 'anthropic') {
    delete body.thinking;
  } else if (provider === 'openai_completions') {
 ...
 * 用于 force_disable_thinking 时拦截上游响应中的思考内容。
 */
