...
function translateReq_Anthropic_to_OpenAIChat(body, modelMapping) {
  ...
  // 遍历 Anthropic 消息，将 system 内容收集到 systemContent 中
  for (const msg of messages) {
    if (msg.role === 'system') {
      const c = typeof msg.content === 'string' ? msg.content : '';
  ...
  // 将 Anthropic 的顶层 system 字段插入为 OpenAI 的第一条消息
  if (systemContent) {
...
function translateStreamEvent_Anthropic_to_OpenAI(event, state) {
  if (!state) state = { id: generateId(), model: '', created: toUnixTimestamp() };
...
function translateStreamEvent_OpenAI_to_Anthropic(event, state) {
  if (!state) state = { msgId: generateId('msg'), model: '', contentBlocks: [], contentIndex: 0 };
...
function translateStreamEvent_Anthropic_to_OpenAIResponses(event, state) {
  if (!state) state = { respId: generateId('resp'), model: '' };
